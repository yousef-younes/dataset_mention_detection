...
from sklearn.metrics import matthews_corrcoef, recall_score, fbeta_score
...
def train(output_dir,seed,writer):
    ...
    for txt in section_txts:
        ...
        tokens = tokens[:382]+tokens[-128:] #tokens[-510:] #tokens[:510] + tokens[-255:]
...
def validation(output_dir, model,tokenizer,validationDataLoader,writer,gpu_ids,epoch_i):
    ...
    if len(acc_res) == 0:
        ...
        if temp_acc > float(acc_res[-1]):
            ...
            model = RobertaForSequenceCalssification.from_pretrained(output_dir)
            model = nn.DataParallel(model, gpu_ids)
            model.cuda()
            #tokenizer = BertTokenizer.from_pretrained(output_dir)
            tokenizer = RobertaTokenizer.from_pretraind(output_dir)
