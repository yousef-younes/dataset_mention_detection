...
'''
The code in this module is behind the results in table 6. It has to be run 4 times one time per line of the table. Each run should have the appropriate
settings which are:
1. the folder for saving the model
2. the folder for saving the visulization data for tensorboard
3. the model to be used (175,178), (403,404), (441,442)
4. the tokenizer assoicated with the model (87,88) (409,410) (446,447)
5. the data part (content or title) to be used lines (62,63) (455,456).
6. name of the final result file 627,628. here the name of the output file along with the experiment results folder must be specified
...
def train(output_dir ,seed ,writer):
    ...
    model = RobertaForSequenceClassificationWithFL.from_pretrained("roberta-base", num_labels=2,
                                                                   output_attentions=False,
                                                                   output_hidden_states=False)
    # model = BertForSequenceClassificationWithFL.from_pretrained('bert-base-uncased',num_labels=2,output_attentions=False,
                                                                #output_hidden_states=False)
    model.gamma = 4
    model.alpha = 0.12
    ...
    for epoch_i in range(0, epochs):
        ...
        for step, batch in enumerate(trainDataLoader):
            ...
            # Accumulate the training loss over all of the batches and gpus so that we can
...
def validation(output_dir, model, tokenizer, validationDataLoader, writer, gpu_ids, epoch_i):
    ...
    if len(fbeta_list) == 0:
        ...
        if temp_fbeta > float(fbeta_list[-1]):
            model_to_save = model.module if hasattr(model, 'module') else model
            model_to_save.save_pretrained(output_dir)
            tokenizer.save_pretrained(output_dir)
            print('Accuracy improved and model saved')
        elif temp_fbeta < float(fbeta_list[-1]):
...
def main():
    ...
    for i in range(len(seeds)):
        ...
        output_dir = exp_path+"/roberta_" + str(seed) + "/"
        print(output_dir)
        # create summary writer for tensorboard
        tensorboard_folder = tensor_path+'/roberta_' +str(i)
...
if __name__ == '__main__':
    ...
    exp_path = 'Roberta_content'
    #this folder save data to be visualized by tensorboard
    tensor_path = "save_roberta_content"
    device, gpu_ids = my_u.get_available_devices()
    main()
    out_file = exp_path+"/roberta_content_results.txt"
    exp_folder = "./"+exp_path+"/roberta_"
